...
# coding: utf-8
#
# Comprehensive data about 900 Emoji characters.
# There are 2 parts to this file:
# - The actual data 
# - regular expressions for manipulating the data
#
# Author: Alex Simonides


# Matches a full line of this dictionary
_pattern = (
    r'("[-0-9a-f]+"):\['
    '(\[[^\]]+\]),'
    '("[^"]*"),'
...
# Same pattern, but the groups are named
_pattern_tagged = (
    r'(?P=ID"[-0-9a-f]+"):\['
    '(?P=UNIFIED\[[^\]]+\]),'
    '(?P=SOFTBANK"[^"]*"),'
    '(?P=GOOGLE"[^"]*"),'
    '(?P=COLONS\[[^\]]+\]),'
    '(?P=CSS_X\d+),(?P=CSS_Y\d+),(?P=UNK_A\d+),(?P=UNK_B\d+)'
    '(?:,?(?P=ASCII(?:"[^"]*")?))\](?P=TAIL,?)')
